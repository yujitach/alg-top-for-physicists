...
Chern-Simons terms. 
\subsubsection{Atiyah-Patodi-Singer index theorem}

\cite{Aharonov:1978gb}

\if0
$(x,y)$ 平面を、$z$ 方向の磁場 $B_z(x,y)$ の影響のもとに動く電子の運動を量子力学的に考えよう。
$(x,y)$ 平面内のの有限の領域 $D$ において $B_z(x,y)$ はノンゼロで一定の値 $b >0 $ を取るが、
その外では $B_z(x,y)=0$ であるとする。全磁場 $ \Phi:=\int_D B_z(x,y) dxdy $ は有限であるとする。

古典的には、十分にエネルギーが低ければ、電子は領域 $D$ 内で円運動をする。よって、量子力学的にも、いくつか束縛状態があるであろうことが予測される。
量子力学的には束縛状態の数はいくつであろうか? 
また、古典的には、スピンが上向きの電子が十分小さい半径の円運動をしていると、
同じ円軌道をスピンが下向きの電子が動くこともできるため、スピンが上向きの束縛状態とスピンが下向きの束縛状態の間には対応がある。
これは、量子力学的にはどうなるだろうか? 
これを、以下の誘導に従って考えよう。

電子の波動関数はスピンの上向き、下向きの二成分をもち \begin{equation}
\vec\psi(x,y)=\begin{pmatrix}
\psi_\uparrow(x,y) \\
\psi_\downarrow(x,y) \\
\end{pmatrix}
\end{equation}とあらわされ、ハミルトニアンは \begin{equation}
H= \left[-i\frac{\partial}{\partial x}+ A_x(x,y)\right]^2 \mathbf{1}
+\left[-i\frac{\partial}{\partial y}+ A_y(x,y)\right]^2 \mathbf{1}
+B_z(x,y)\sigma_z
\end{equation} とかける。
ただし、簡単のため $m=\hbar=e=1$ なる単位系をとる。
...
\begin{itemize}
\item[1.]
ハミルトニアン $H$ とスピンの $z$ 成分 $\sigma_z$ は同時対角化できることを示せ。
\item[2.] 
次のように定義される演算子 $D$ \begin{equation}
D=\sigma_x \left[-i\frac{\partial}{\partial x} + A_x(x,y) \right]
+\sigma_y \left[-i\frac{\partial}{\partial y}+A_y(x,y)\right]
\end{equation} が、\begin{equation}
H=D^2, \qquad [D,\sigma_z ]= -\sigma_z 
\end{equation}を満たすことを示せ。
\item[3.] 束縛状態 $\vec \psi(x,y)$ がエネルギーが $E$ およびスピン の$z$ 成分が $s_z$ の固有状態であるとする。
	\begin{enumerate}
	\item $E=0$ である必要十分条件が $D\vec\psi(x,y)=0$ であることを示せ。
	\item $E\neq 0$ だとする。$D\vec\psi(x,y)$ はまたエネルギーおよびスピン $z$ の固有状態であるが、それぞれ固有値を求めよ。
	\item $E\neq 0$ とする。$E$ が正であることを示せ。
	\end{enumerate}
\item[4.] ベクトルポテンシャルを\begin{equation}
A_x(x,y)=\frac{\partial\phi}{\partial y}, 
A_y(x,y)=-\frac{\partial\phi}{\partial x}
\end{equation} という関係を満たす関数 $\phi(x,y)$ を用いて表わそう。
\begin{enumerate}
	\item $B_z(x,y)$ と $\phi(x,y)$ のみたす微分方程式を書きくだせ。
	\item このとき、$r=\sqrt{x^2+y^2}$ が大きなところでは、$e^{\phi(x,y)}$ は\begin{equation}
e^{\phi(x,y)} \sim r^c
\end{equation} と振る舞った。$c$ を求めよ。
	\end{enumerate}
\item[5.] ゼロエネルギー固有状態を決定しよう。そのためには、上でみたように、$D\vec\psi(x,y)=0$ を解けばよい。
	\begin{enumerate}
	\item $f_\uparrow(x,y)$, $f_\downarrow(x,y)$ を\begin{equation}
	\psi_\uparrow(x,y)=f_\uparrow(x,y) e^{\phi(x,y)}, \qquad
	\psi_\downarrow(x,y)=f_\downarrow(x,y) e^{-\phi(x,y)}, 
\end{equation} によって定義する。$D\vec\psi(x,y)=0$ から、$f_\uparrow(x,y)$, $f_\downarrow(x,y)$ に対する方程式を求めよ。
	\item これより、$f_\uparrow(x,y)$ は $ z=x+iy$ の正則関数、
	$f_\downarrow(x,y)$ は $z$ の反正則関数であることを示せ。
	\item 束縛状態の波動関数は二乗可積分であることに注意して、スピンが上向きでエネルギーがゼロの状態はないことを示せ。
	\item 同様にして、スピンが下向きでエネルギーがゼロの束縛状態の一次独立なものの数を、全磁束 $\Phi$ を用いてあらわせ。これを $N$ とする。
	\end{enumerate}
\item[6.] スピンが上向きの束縛状態の一次独立なものの数が $N_\uparrow$、
スピンが下向きの束縛状態の一次独立なものの数が $N_\downarrow$ だとする。
小問 3. に留意して、$N_\uparrow-N_\downarrow$ を求めよ。
\fi
