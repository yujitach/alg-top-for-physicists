...
It is also common to introduce a norm on the fibers $V$ of a vector bundle,
...
Given a vector space $V$, the space of linear functions $V\to \bK$ is denoted by $V^\wedge$
and is also a  vector space.
Doing this fiberwise, we have:
\begin{definition}
  Given a vector bundle $E\to B$ with fiber $V$,
  we can form the dual bundle $E^\wedge\to B$ whose fiber is $V^\wedge$.
\end{definition}
...
\end{definition}
This is the operation, given a family of Hilbert spaces parameterized over 
a parameter space $B$,
the family of spaces of observables acting on each Hilbert space, over the same parameter space $B$.
...
\begin{definition}
  Given a manifold $M$ of dimension $n$, the tangent bundle $TM\to M$ is a real vector bundle 
  whose fiber is $\bR^n$,
