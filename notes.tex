...
This is also irreducible as an ungraded representation: \begin{equation}
V_{2n}^{\bZ_{2}}=V_{2n}.
\end{equation}
Now, the irreducible graded representation $V_{2n-1}^{\bZ_2}$ of $\Cliff_\bC(2n-1)$ 
is also unique and  of dimension $2^n$, and is simply the same $V$ with the action of $\psi_{2n}$ forgotten: \begin{equation}
V_{2n-1}^{\bZ_2}=V_{2n}^{\bZ_{2}}
\end{equation}
But this is \emph{not irreducible} as an ungraded representation of $\Cliff_\bC(2n-1)$,
...
But over complex numbers the convention does not matter (as we can multiply any generator by $i$).

\paragraph{When $D$ is even, $D=2n$:}
Suppose $D=2n$.
Then $\psi$ takes values in the ungraded Clifford representation  $V_{2n}$ of dimension $2^n$. 
Let \begin{equation}
\Gamma:= \gamma^0 \gamma^1\cdots \gamma^d. 
\end{equation} This squares to a scalar $\Gamma^2 \propto 1$,
and commute with the Lorentz generators $\gamma^\mu \gamma^\nu$.
Therefore we can split $V_{2n}$ into two irreducible representations $S_+$ and $S_-$ of $so(1,2n-1)$,
each of dimension $2^{n-1}$, distinguished by the value of $\Gamma$.
They are known as the chiral or Weyl spinors,
whereas $V_{2n}$ itself is known as the Dirac spinor.

\paragraph{When $D$ is odd, $D=2n-1$:}
Suppose $D=2n$.
Then $\psi$ takes values in the ungraded Clifford representation  $V_{2n-1}$ of dimension $2^{n-1}$. 
Here, considering $\Gamma$ as above does not help,
since $\Gamma$ commute with every generator $\gamma^0$ to $\gamma^d$,
and as such, $\Gamma$ is a scalar already on $V_{2n-1}$.
This is the spinor representation of $so(1,d)$.
...
\end{equation} where $\eta^{\mu\nu}$ is $(+--\cdots--)$ or $(-++\cdots++)$ 
depending on the convention.
Then $\psi$ is an \emph{ungraded} representation of $\Cliff_\bR(1,d)$ or $\Cliff_\bR(d,1)$.
Luckily, as we learned above, the convention on $\eta^{\mu\nu}$ does not matter,
since there is at least one $+$-type Clifford generator. 

As in the complex case, an ungraded Clifford representation 
does not necessarily give an irreducible representation of $so(1,d)$.
This can happen, as above, when $D=2n$ is even, 
but there is one additional constraint. 
The operator $\Gamma:=\gamma^0\gamma^1 \cdots \gamma^{d}$ 
satisfies  to $\Gamma^2=+1$ when $D\equiv 2$ mod $4$ 
but $\Gamma^2=-1$ when $D\equiv 0$ mod $4$.
On a real vector space, an operator $\Gamma$ with $\Gamma^2=-1$ does not have eigenspaces.
So, the real spinor can only be made chiral when $D\equiv 2$ mod $4$.
...
\hline
2 & \bC & \bC & 1 & \bR & \bR & 1\\
3 & \multicolumn{2}{c|}{\bC^2} & 2 &  \multicolumn{2}{c|}{\bR^2} & 2\\
4 & \bC^2 & \bC^2 & 2 & \multicolumn{2}{c|}{\bC^2} & 4\\
5 & \multicolumn{2}{c|}{\bC^4} & 4 &  \multicolumn{2}{c|}{\bH^2} & 8\\
6 & \bC^4 & \bC^4 & 4 &  \bH^2 & \bH^2 & 8\\
7 & \multicolumn{2}{c|}{\bC^8} & 8 &  \multicolumn{2}{c|}{\bH^4} & 16\\
8 & \bC^8 & \bC^8 & 8  &  \multicolumn{2}{c|}{\bC^8}  & 16\\
9 & \multicolumn{2}{c|}{\bC^{16}} & 16  &  \multicolumn{2}{c|}{\bR^{16}}  & 16\\
10 & \bC^{16} & \bC^{16} & 16  &  \bR^{16}  & \bR^{16} & 16\\
11 & \multicolumn{2}{c|}{\bC^{32}} & 32 &  \multicolumn{2}{c|}{\bR^{32}} & 32\\
\end{array}
\]
\begin{itemize}
\item $D$ is the relativists' dimension.
\item spinor$_{\bC,\bR}$ shows the minimal spinor representations in each dimensions.
If there are two chiral representations distinguished by the eigenvalue of $\Gamma$,
 they are shown separately. 
\item $\dim_{\bC,\bR}$ shows the dimensions of the minimal spinor representations.
\item In the case of complex spinors, if there are $n$ fermions of the same type, the flavor symmetry is $u(n)$.
\item In the case of real spinors, if there are $n$ fermions of the same type,
 the flavor symmetry is $so(n)$, $u(n)$ or $sp(n)$
 depending on the types of spinors shown in the column spinor$_\bR$ are
