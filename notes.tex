...
\begin{equation}
  \begin{array}{cccccccc}
    U_i \times G &\supset& U_{ij}\times G & \xrightarrow{f_{ij}} & 
...
The sphere $S^2$ parameterized by $(X,Y,Z)$ is often referred to as the Bloch sphere.
...
\begin{definition}
  ...
  Furthermore, $G\to G/H$ is a principal $H$-bundle.
\end{definition}

A common way this appears in physics is via symmetry breaking.
Say we have a configuration space $M$ acted on by a symmetry group $G$ from the left:
$M\ni m\mapsto gm\in M$ for $g\in G$.
Suppose we have a potential $V:M\to \bR$ invariant under the $G$ action, i.e.~$V(gm)=V(m)$.
...
\begin{example}
  There is a fiber bundle $SO(n-1)\to SO(n) \to S^{n-1}$.
\end{example}
...
\begin{example}
There is a fiber bundle $U(n-1)\to U(n)\to S^{2n-1}$.
\end{example}
...
and we found that:
\begin{example}
  There is a fiber bundle $Sp(n-1)\to Sp(n)\to S^{4n-1}$.
\end{example}

Let us now consider $g\in SU(2)$ acting on $\bC^2$.
...
An invariant function $V:\bR^3\to \bR$ again has the form $V(x,y,z)=f(r)$
...
\begin{example}
  ...
  $
\end{example}
Recalling that $SU(2)\simeq S^3$, we see that $SO(3)\simeq \RP^3$.

Let us check that this bundle is not a trivial product and therefore $SU(2) \neq \{\pm1\}\times SO(3)$.
For this, it suffices to restrict our attention to the subgroup
$U(1) \subset SU(2)$ we encountered just above,
consisting of elements $g(\theta):=e^{i\theta\sigma_z}$.
$R_{g(\theta)}$ can be easily computed to be a rotation around the $z$-axis by an angle $2\theta$.
Then, restricted to this subspace, we have the fibration \begin{equation}
  \{\pm1\}  \to S^1 \to S^1
\end{equation}
where the second map is 2:1, which is not a trivial product.

There is another common example we see in quantum mechanics.
...
Summarizing, 
\begin{example}
  There are fiber bundles $U(1)\to U(n)\to PU(n)$
  and $\bZ_n \to SU(n) \to PU(n)$.
\end{example}
...
\begin{figure}[h]
  ...
  \]
    ...
    \label{fig:3He}
\end{figure}
A $^3$He atom is a fermion, which can form a Cooper pair at low temperature.
There are two superfluid phases with zero magnetic phase, known as the A phase and the B phase.
With magnetic field, there is also the phase known as the A$_1$ phase.
The order parameter (the vacuum expectation value of the Cooper pair) takes values in
$\bC^3\otimes \bC^3$,
where the first factor is the orbital angular momentum part
and the second factor is the spin angular momentum part,
both with spin 1.
Denote the basis vectors of the first $\bC^3$ by $\mathbf{e}_{1,2,3}$
and those of the second by $\mathbf{f}_{1,2,3}$.
The symmetry group $G$ of the potential is two separate $SO(3)$ actions on 
$\mathbf{e}_{1,2,3}$ and $\mathbf{f}_{1,2,3}$,
together with a common $U(1)$ rotation $\mathbf{e}_i \otimes \mathbf{f}_j \mapsto c\, \mathbf{e}_i \otimes \mathbf{f}_j$,
...
\begin{itemize}
\item $\mathbf{e}_1 \otimes \mathbf{f}_1$ in the A phase,
\item $\mathbf{e}_1 \otimes (\mathbf{f}_2+ i\mathbf{f}_3)$ in the A$_1$ phase,
\item $\sum_i \mathbf{e}_i \otimes \mathbf{f}_i$ in the B phase.
\end{itemize}
Denoting the subgroup preserved by these order parameters by $H_\text{A, A$_1$, B}$ respectively,
the space of the potential minimum is given by $G/H_\text{A, A$_1$, B}$.
\end{example}
...
\begin{definition}
  ...
  and $U_j\subset B$ are related in the following manner on the overlap $U_{ij}=U_i\cap U_j$:
  \begin{equation}
  \begin{array}{cccccccc}
    U_i \times V &\supset& U_{ij}\times V & \xrightarrow{f_{ij}} & 
    U_{ij}\times V & \subset & U_j\times V \\
    \downarrow & & \downarrow & & \downarrow & & \downarrow \\
    U_i & \supset & U_{ij} & = & U_{ij} & \subset & U_j
  \end{array}
  \end{equation}
  where $f_{ij}$ is given by \begin{equation}
    f_{ij}: (b,v) \mapsto (b, g_{ij}(b) v)
    \label{eq:vec-bundle-transition-functions}
  \end{equation} where $g_{ij}(b): V\to V$ is a linear map.
  Th dimension of $V$ is called the rank of the vector bundle.
\end{definition}
We can consider $\bR$-vector bundles, $\bC$-vector bundles, and $\bH$-vector bundles.

In physics it is also common to have an inner product on the fibers $V$ of a vector bundle,
so that the transition functions $g_{ij}(b)$ are not just linear maps
...
\begin{itemize}
  ...
  \item As wavefunctions, where the system can be separated 
  into the degrees of freedom along the base space $B$
  and additional degrees of freedom parameterized by the fiber $V$ 
  which is the Hilbert space of an additional quantum system.
\end{itemize}
...
On a completely general complex vector bundle,
the transition functions $g_{ij}(b)$ are in $GL(n,\bC)$,
...
Then we can consider the principal $U(n)$-bundle associated to the vector bundle, defined via \eqref{eq:principal-bundle-transition-function}.
The fiber is now $U(n)$ instead of $\bC^n$.

In high-energy physics we often encounter the situation where $g_{ij}(b)$ is in a subgroup $G$ of $U(n)$;
...
\begin{definition}
  A $\bK$-vector bundle whose transition functions $g_{ij}(b)$ are in a subgroup $G$ of $GL(n,\bK)$
  is called to have the structure group $G$.
  The principal $G$-bundle given by the same gransition functions are called the associated principal $G$-bundle.
\end{definition}

There is an inverse operation to this.
Namely, given a principal $G$-bundle over $B$,
we have a set of transition functions $g_{ij}$ in $G$
between $U_i\times G$ and $U_j\times G$,
as described in \eqref{eq:principal-bundle-transition-function}.
Now, pick a linear action $\rho: G \curvearrowright V$ of $G$ on a vector space $V$.
...
are given by $\rho(g_{ij})$.

\begin{definition}
  The vector bundle constructed as above is called an associated vector bundle to the principal $G$-bundle,
  determined by the representation $\rho$.
\end{definition}

\subsection{Tangent and related bundles}
...
With a metric, we can form an orthogonal basis of the tangent space at each point.
