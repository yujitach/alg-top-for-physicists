...
\begin{proposition}
If $X$ and $Y$ are homotopy equivalent, then $\pi_n(X)\simeq \pi_n(Y)$ for all $n\ge 0$.
\end{proposition}

So, if we can compute $\pi_n(X)$ and $\pi_n(Y)$ for some $n$ and show $\pi_n(X)\neq \pi_n(Y)$, 
then $X$ and $Y$ are not homotopy equivalent.
...
Homotopy groups of spheres $\pi_{n+k}(S^n)$ 
for small $n$ and $k$ are tabulated in Table~\ref{tab:pi_nS^m}.
The data are taken from \cite[付録, 公式 7, V]{Jiten}.
...
This is now known to $\lim_{n\to\infty}\pi_{n+83}(S^n)$, \cite{Isaksen}.
