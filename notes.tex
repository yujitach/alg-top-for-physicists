...
Let us discuss a common example we see in quantum mechanics.
...
where two $\bC^2$ factors are the double representations of $SU(2)$.
A more convenient way to think about this action
is to regard $\bC^2\otimes \bC^2$ as the space of 
complex two-by-two matrices $M(2,\bC)$,
and to let $(g,g')\in SU(2)\times SU(2)$ to act on $m\in M(2,\bC)$
via \begin{equation}
m\mapsto g m g'{}^{-1}.
\end{equation}
...
i.e.~the subspace $SU(2)\subset M(2,\bC)$.
We thus found:
\begin{example}
...
$\bC^3\otimes \bC^3$,
where the first factor is the spin angular momentum part
and the second factor is the orbital angular momentum part,
both with spin 1.
...
\end{example}
Note that the pattern of symmetry breaking is very similar to the 
