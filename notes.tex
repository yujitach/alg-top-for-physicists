...
\end{equation} in the relativistic convention, or \begin{equation}
\frac{\partial}{\partial t} \psi +\alpha_1 \frac{\partial} {\partial x}\psi  + m\beta\psi=0
\end{equation} in the non-relativistic convention.
As $\beta\sim \gamma_0$  and $\alpha\sim \gamma_0\gamma_1$, 
...
which splits into $\bR_+$ and $\bR_-$ of $\Cliff_\bR(1)$ when we turn off the mass/gap.
...
\end{pmatrix}.
\end{equation}
$m$ is then a real parameter. We have a disperson relation \begin{equation}
...
1& 0
\end{pmatrix}
\end{equation} for pin$^+$ with $T^2=(-1)^F$.

Now suppose you want to make this massive/gapped. 
We write \begin{equation}
\gamma_0 \partial_t \psi + \gamma_1 \partial_x \psi + m\psi=0.
\end{equation} The fact that $\psi$ is a real two-component vector 
forces us to take $m$ to be real.
To allow $\psi\sim e^{\pm i mt}$ when $\psi$ is $x$-independent, 
$\gamma_0$ needs to have eigenvalue $\pm i$.
This means that this pair of a left-moving fermion and a right-moving fermion
can be made massive for pin$^+$ with $T^2=(-1)^F$,
but this is impossible for pin$^-$ with $T^2=+1$.
(This is opposite to what should be the case. ???)
