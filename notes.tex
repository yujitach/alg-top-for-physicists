...
the eigenvalues of $D$ are $\lambda_n = n+(1/2-a)$ for $n\in \bZ$,
where \begin{equation}
\frac12 -a :=\int_{0}^{2\pi} A d\theta  .
\end{equation}
...
Now, assuming that $a$ is sufficiently small, $n+1/2-a$ for $  n\ge 0$ is positive, whereas $n+1/2-a$ for $n<0$ is negative.
Then we have \begin{equation}
\sum_{n\ge 0} (+1) e^{-s(n+1/2-a)} = 1/s+a+ O(s),\qquad
\sum_{n<0} (-1)e^{-s|n+1/2-a|}= 1/s-a+O(s).
\end{equation}
So we have \begin{equation}
  \eta(D)=\lim_{s\to 0}\frac12 (2a+O(s)) = a.
\end{equation}
That $\eta(D)=0$ for $a=0$ is consistent because this should be the phase of a
thermal partition function, which should be real and positive.

If $a$ is not necessarily small, we need to find an integer $m$ such that $|a-m| \le 1/2$.
Then for $a':=a-m$, the spectrum $\{n+(1/2-a)\}$ is the same as $\{n+(1/2-a')\}$,
so the computation above goes through, and we find 
\begin{equation}
\eta(D) = a' = a - m \in [-1/2,1/2].
\end{equation}
In this manner we find that $\eta(D)$ is not necessarily continuous as a real-valued function but it is continuous modulo 1.
