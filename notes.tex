...
That the magnetic charge is quantized was first discovered by Dirac \cite{Dirac}.
...
We can also have ``solitons with core'' 
classified by $\pi_{d-1}(X)$,
where something drastic necessarily happen in the core.
This costs infinite energy in the asymptotic region unless some action 
$G\curvearrowright X$ is appropriately gauged.

\begin{example}
Abrikosov/Nielsen-Olsen vortex, codimension 2, $X=U(1)$
\end{example}

\begin{example}
`Instanton particle', codimension 4, $X=SU(2)$
\end{example}

\begin{example}
't Hooft-Polyakov monopole , codimension 3, $X=SU(2)/U(1)$ or $ X=SO(3)/SO(2)$
\end{example}
